...
namespace imgui {
int guiMain(GuiMgrBase* guiMgr,
            const wchar_t* title,
            int x,
            int y,
            int width,
            int height) {
  TCHAR className[] = _T("imgui-dx9-wndclass");
  ...
  WNDCLASSEX wc = {sizeof(WNDCLASSEX),
                   ...
                   NULL};
  RegisterClassEx(&wc);
  HWND hwnd = CreateWindow(className, title, WS_OVERLAPPEDWINDOW, x, y, width,
  ...
  while (msg.message != WM_QUIT) {
    ...
    D3DCOLOR clear_col_dx =
        D3DCOLOR_RGBA((int)(guiMgr->clearCol()->x * 255.0f),
                      (int)(guiMgr->clearCol()->y * 255.0f),
                      (int)(guiMgr->clearCol()->z * 255.0f),
                      (int)(guiMgr->clearCol()->w * 255.0f));
