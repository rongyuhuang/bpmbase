...
void ImGui_ImplDX9_RenderDrawLists(ImDrawData* draw_data) {
  ...
  for (int n = 0; n < draw_data->CmdListsCount; n++) {
    ...
    for (int i = 0; i < cmd_list->VtxBuffer.size(); i++) {
      ...
      vtx_dst->pos[2] = 0.0f;
      vtx_dst->col =
          (vtx_src->col & 0xFF00FF00) | ((vtx_src->col & 0xFF0000) >> 16) |
  ...
  // aren't relying on D3DXMatrixIdentity()/D3DXMatrixOrthoOffCenterLH() or
  // DirectX::XMMatrixIdentity()/DirectX::XMMatrixOrthographicOffCenterLH()
  {
    const float L = 0.5f, R = io.DisplaySize.x + 0.5f, T = 0.5f,
...
bool ImGui_ImplDX9_Init(void* hwnd, IDirect3DDevice9* device) {
  ...
  ImGuiIO& io = ImGui::GetIO();
  io.KeyMap[ImGuiKey_Tab] = VK_TAB;  // Keyboard mapping. ImGui will use those
  ...
  io.RenderDrawListsFn =
      ImGui_ImplDX9_RenderDrawLists;  // Alternatively you can set this to NULL
...
void ImGui_ImplDX9_InvalidateDeviceObjects() {
  ...
  if (LPDIRECT3DTEXTURE9 tex =
          (LPDIRECT3DTEXTURE9)ImGui::GetIO().Fonts->TexID) {
    tex->Release();
...
void ImGui_ImplDX9_NewFrame() {
  ...
  if (deltaTime > 1.0f / 60.0f) {
    ...
    Sleep((1.0f / 60.0f - deltaTime) * 1000);
    QueryPerformanceCounter((LARGE_INTEGER*)&current_time);
    io.DeltaTime = (float)(current_time - g_Time) / g_TicksPerSecond;
    g_Time = current_time;
  }
