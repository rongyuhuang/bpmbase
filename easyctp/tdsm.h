...
public:
    //CTP部分
    std::string brokerID,tdFront,userID,password,tdFlowPath;
    int apiTimeout,orderFreq=6,queryFreq=1;
...
private:
    ...
    std::atomic_int reqID;
    std::mutex flowMutex;
    std::chrono::system_clock::time_point tpLastChecker;
    std::chrono::milliseconds orderDuration;
    std::chrono::milliseconds queryDuration;
private:
    void login();
