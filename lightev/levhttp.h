...
class EvHttpRequest {
 public:
  EvHttpRequest(struct evhttp_request* req) { mReq = req; }
  ~EvHttpRequest() {}
  ...
  inline void sendReply(int responsecode,
  ...
  inline void cancel() { evhttp_cancel_request(mReq); }
  ...
  inline const char* uriStr() { return evhttp_request_get_uri(mReq); }
  inline EvHttpUri uri() {
    EvHttpUri u(evhttp_request_get_evhttp_uri(mReq));
    return u;
  }

  inline const char* host() { return evhttp_request_get_host(mReq); }
  inline enum evhttp_cmd_type cmd() { return evhttp_request_get_command(mReq); }
  inline int responseCode() { return evhttp_request_get_response_code(mReq); }
...
class EvHttpServer {
 public:
  ...
  static std::string encodeUriString(const char* src) {
    std::string s;
  ...
  static std::string decodeUriString(const char* src) {
    std::string s;
  ...
  static std::string htmlEscape(const char* src) {
