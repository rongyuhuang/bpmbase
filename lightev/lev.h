...
// HACK: access internal function to prevent base loop from exiting if there are
// no events
extern "C" void event_base_add_virtual_(struct event_base*);
extern "C" void event_base_del_virtual_(struct event_base*);
...
class IpAddr {
 public:
  ...
  inline bool assign(const char* addrandport) {
    ...
    int ret = evutil_parse_sockaddr_port(addrandport, (struct sockaddr*)&mAddr,
                                         &mSize);
    return (ret == 0);
  }
  ...
  inline bool assign(const char* addrstr, uint16_t port) {
    ...
    int ret =
        evutil_parse_sockaddr_port(addrstr, (struct sockaddr*)&mAddr, &mSize);
  ...
  std::string toString() const { return makeStr(false); }
  ...
  inline int addrLen() const { return mSize; }
 ...
 protected:
  ...
  std::string makeStr(bool showport) const {
    ...
    char abuf[64];
    buf[0] = '\0';
    if (evutil_inet_ntop(AF_INET, &(((struct sockaddr_in*)&mAddr)->sin_addr),
                         abuf, sizeof(abuf))) {
      if (!showport) {
...
class EvEvent {
 public:
  EvEvent() : mPtr(NULL), mUserData1(NULL), mUserData2(NULL) {}
  ~EvEvent() { free(); }
  ...
  inline void* userData1() { return mUserData1; }
  inline void setUserData1(void* userdata1) { mUserData1 = userdata1; }
  inline void* userData2() { return mUserData2; }
  inline void setUserData2(void* userdata2) { mUserData2 = userdata2; }
  ...
  void newSignal(event_callback_fn callback,
                 int signum,
  ...
  void newSignalCtx(event_callback_fn callback,
                    int signum,
                    struct event_base* base,
                    void* ctx) {
    free();
  ...
  inline void start() { event_add(mPtr, NULL); }
  inline void start(int msecs) {
    timeval t = EvEvent::tvMsecs(msecs);
    event_add(mPtr, &t);
  }
  inline void end() { event_del(mPtr); }

  inline void activateUser(int res) { event_active(mPtr, res, 0); }

  inline struct event_base* base() { return event_get_base(mPtr); }

  inline void exitLoop() { event_base_loopexit(base(), NULL); }
...
class EvBuffer {
 public:
  EvBuffer() : mPtr(NULL), mOwner(false) {}
  EvBuffer(struct evbuffer* ptr) : mPtr(ptr), mOwner(false) {}
  ~EvBuffer() { free(); }
  void newBuffer() {
    free();
    mPtr = evbuffer_new();
  ...
  inline void enableLocking() { evbuffer_enable_locking(mPtr, NULL); }
  inline void lock() { evbuffer_lock(mPtr); }
  inline void unlock() { evbuffer_unlock(mPtr); }
  inline size_t length() { return evbuffer_get_length(mPtr); }
  inline size_t space() { return evbuffer_get_contiguous_space(mPtr); }
  ...
  inline struct evbuffer* ptr() { return mPtr; }
...
class EvBufferEvent {
 public:
  EvBufferEvent() : mPtr(NULL), mOwner(false) {}
  EvBufferEvent(struct bufferevent* ptr) : mPtr(ptr), mOwner(false) {}
  ~EvBufferEvent() { free(); }
  void free() {
    if (mOwner && mPtr) {
  ...
  inline void own(bool objowns) { mOwner = objowns; }

  bool connect(const IpAddr& sa) {
    int ret =
        bufferevent_socket_connect(mPtr, (sockaddr*)sa.addr(), sa.addrLen());

    return (ret == 0);
  }

  inline EvBuffer input() { return EvBuffer(bufferevent_get_input(mPtr)); }
  inline EvBuffer output() { return EvBuffer(bufferevent_get_output(mPtr)); }

  inline void enable(short flags) { bufferevent_enable(mPtr, flags); }
  inline void disable(short flags) { bufferevent_disable(mPtr, flags); }

  void setTcpNoDelay() {
    int one = 1;
    setsockopt(bufferevent_getfd(mPtr), IPPROTO_TCP, TCP_NODELAY,
...
class EvConnListener {
 public:
  EvConnListener() : mPtr(NULL), mOwner(false) {}
  EvConnListener(struct evconnlistener* ptr) : mPtr(ptr), mOwner(false) {}
  ~EvConnListener() { free(); }
  void free() {
    if (mOwner && mPtr) {
  ...
  void newListener(const IpAddr& sa,
    ...
    mPtr = evconnlistener_new_bind(base, callback, cbarg, flags, backlog,
                                   sa.addr(), sa.addrLen());
    mOwner = true;
  }

  inline void enable() { evconnlistener_enable(mPtr); }
  inline void disable() { evconnlistener_disable(mPtr); }
  inline struct event_base* base() { return evconnlistener_get_base(mPtr); }
...
class EvBaseLoop {
 public:
  ...
  inline operator struct event_base*() { return mBase; }

  inline struct event_base* base() { return mBase; }

  inline void incLoopRef() { event_base_add_virtual_(mBase); }

  inline void decLoopRef() { event_base_del_virtual_(mBase); }
  ...
  void loopbreak() { event_base_loopbreak(mBase); }

  void loopcontinue() { event_base_loopcontinue(mBase); }

  void loopexit(const struct timeval* tv = NULL) {
    event_base_loopexit(mBase, tv);
  }

  static void enableDebug() { event_enable_debug_mode(); }
...
class EvKeyValues {
 public:
  EvKeyValues() : mHdrHead(NULL), mHdrPtr(NULL), mOwner(false) {}
  EvKeyValues(struct evkeyvalq* headers) : mHdrHead(headers), mOwner(false) {
    moveFirst();
  }
  ...
  inline bool eof() { return mHdrPtr == NULL; }
  inline const char* key() { return (mHdrPtr != NULL) ? mHdrPtr->key : NULL; }
...
class EvHttpUri {
 public:
  EvHttpUri() : mUri(NULL), mOwner(false) {}
  EvHttpUri(struct evhttp_uri* uri) : mOwner(false) { mUri = uri; }
  EvHttpUri(const struct evhttp_uri* uri) : mOwner(false) {
    mUri = (struct evhttp_uri*)uri;
  }
  EvHttpUri(struct evhttp_request* req) : mOwner(false) {
  ...
  void newParsed(const char* sourceuri, bool nonconformant) {
    ...
    free();
    mOwner = true;
    mUri = evhttp_uri_parse_with_flags(
        sourceuri, nonconformant ? EVHTTP_URI_NONCONFORMANT : 0);
  }
  void free() {
    if (mOwner && mUri) {
  ...
  inline const char* scheme() { return evhttp_uri_get_scheme(mUri); }
  inline const char* userInfo() { return evhttp_uri_get_userinfo(mUri); }
  inline const char* host() { return evhttp_uri_get_host(mUri); }
  inline int port() { return evhttp_uri_get_port(mUri); }
  inline const char* path() { return evhttp_uri_get_path(mUri); }
  inline const char* query() { return evhttp_uri_get_query(mUri); }
  inline const char* fragment() { return evhttp_uri_get_fragment(mUri); }
  ...
  bool setPort(int port) { return evhttp_uri_set_port(mUri, port) == 0; }
