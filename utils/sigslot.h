...
#ifndef VDK_SIGSLOT_H
#define VDK_SIGSLOT_H
...
class signal<_ResT(_ArgTs...)> final {
    ...
    enum class sync_stage : char { sync_stage_1 = 1,
        sync_stage_2 = 2 };
    ...
    class slot_functor final {
        ...
        {
            return (
                reinterpret_cast<const target_slot<_Class, _Signature>*>(data)->mp_instance->*reinterpret_cast<const target_slot<_Class, _Signature>*>(data)->mp_function)(
                args...);
        }
        ...
        static _ResT invoke(const byte* const data, _ArgTs&... args)
        {
            return (*reinterpret_cast<const target_slot<_Class, ::std::nullptr_t>*>(
    ...
    public:
              ...
              mp_invoker{ nullptr }
        {
            ...
            auto storage = reinterpret_cast<target_slot< ::std::nullptr_t, _Signature>*>(
                &m_target[0]);
        ...
        slot_functor(_Class* object, _Signature method) noexcept
            : m_target{},
              mp_invoker{ nullptr }
        {
            auto storage = reinterpret_cast<target_slot<_Class, _Signature>*>(&m_target[0]);
        ...
        explicit slot_functor(_Class* functor) noexcept : m_target{},
                                                          mp_invoker{ nullptr }
        {
            auto storage = reinterpret_cast<target_slot<_Class, ::std::nullptr_t>*>(
                &m_target[0]);
        ...
        {
            ::std::copy(other.m_target, other.m_target + target_size, m_target);
    ...
    public:
        ...
        connection(const slot_functor& slot,
            ...
            bool trackable) noexcept : m_slot(slot),
                                       m_track_ptr(t_ptr),
                                       mp_next(nullptr),
                                       mp_deleted(nullptr),
    ...
    class storage final {
        ...
        void expand()
        {
            connection* new_block = reinterpret_cast<connection*>(
                VDK_NEW(sizeof(connection) * m_capacity + sizeof(connection*)));
            connection** next_block = reinterpret_cast<connection**>(mp_block + m_capacity);

            while (*next_block) {
                next_block = reinterpret_cast<connection**>((*next_block) + m_capacity);
            }

            (*next_block) = new_block;
            init(new_block);
            (*reinterpret_cast<connection**>(new_block + m_capacity)) = nullptr;
        }
        ...
        {
            if (mp_block) {
                connection* to_delete = (*reinterpret_cast<connection**>(mp_block + m_capacity));

                while (to_delete) {
                    connection* block = (*reinterpret_cast<connection**>(to_delete + m_capacity));
    ...
    public:
            ...
            , m_capacity(capacity >= 1 ? capacity : 1)
        {
            mp_block = reinterpret_cast<connection*>(
                VDK_NEW(m_capacity * sizeof(connection) + sizeof(connection*)));
            init(mp_block);
            (*reinterpret_cast<connection**>(mp_block + m_capacity)) = nullptr;
        }

        // Move-construct storage
        storage(storage&& other) noexcept : mp_block(other.mp_block),
    ...
    public:
        // Construct auto_lock
        explicit auto_lock(slim_lock& s_lock) noexcept : mp_lock(&s_lock)
        {
            mp_lock->lock();
        }

        // Move-construct auto_lock
        auto_lock(auto_lock&& other) noexcept : mp_lock(other.mp_lock)
        {
    ...
    public:
        ...
        explicit read_guard(counter_type& counter) noexcept : mp_counter(&counter)
        {
            ++(*mp_counter);
        }

        // Move-construct read_guard
        read_guard(read_guard&& other) noexcept : mp_counter(other.mp_counter)
        {
    ...
    {
        return read_guard(m_sync_stage.load() == sync_stage::sync_stage_1
                ? m_access_s1
                : m_access_s2);
    }

    // Access signal's internal structure for writing
    auto_lock write_access() const noexcept { return auto_lock(m_write_lock); }
...
public:
    ...
    template <typename _Class, typename _Signature>
    bool disconnect(::std::shared_ptr<_Class> object,
        _Signature method) noexcept
    {
        auto writer = write_access();
    ...
    template <typename _Class, typename _Signature>
    bool connected(::std::shared_ptr<_Class> object, _Signature method) const
        noexcept
    {
        auto reader = read_access();
    ...
    // Block signal
    void block(bool block = true) noexcept { m_blocked.store(block); }
    ...
    bool empty() const noexcept { return !mp_first_slot.load(); }
...
public:
    ...
    explicit signal(size_type capacity = 5)
        ...
        , m_blocked(false)
    {
        mp_block = reinterpret_cast<connection*>(VDK_NEW(m_capacity * sizeof(connection)));
    ...
    // Block signal
    void block(bool block = true) noexcept { m_blocked = block; }

    // Check whether signal is blocked
    bool blocked() const noexcept { return m_blocked; }

    // Emit signal
    void emit(_ArgTs... args) { activate(args...); }

    // Emit signal
    void operator()(_ArgTs... args) { activate(args...); }
    ...
    // Check whether list of connected slots is empty
    bool empty() const noexcept { return m_first_slot == null_index; }
...
::std::shared_ptr<_Ty> create_shared_object(_ArgTs&&... args)
{
    ...
    ::std::shared_ptr<_Ty> sh_ptr(address, [](_Ty* ptr) {
        ptr->~_Ty();
        VDK_DELETE(ptr);
    });
...
::std::shared_ptr<_Ty> allocate_shared_object(const _Alloc& alloc,
    _ArgTs&&... args)
{
    _Alloc _allocator(alloc);
    _Ty* address = _allocator.allocate(1);
    ::std::shared_ptr<_Ty> sh_ptr(address, [_allocator](_Ty* ptr) mutable {
