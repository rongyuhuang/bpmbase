...
class LockImpl {
public:
    LockImpl() { ::InitializeCriticalSection(&critical_section_); }
    ~LockImpl() { ::DeleteCriticalSection(&critical_section_); }
    bool Try() { return ::TryEnterCriticalSection(&critical_section_) != FALSE; }
...
void InitLogging(const wchar_t* new_log_file,
    ...
    OldFileDeletionState delete_old)
{
    ...
    if (logging_destination == LOG_NONE || logging_destination == LOG_ONLY_TO_SYSTEM_DEBUG_LOG)
        return;
...
void DisplayDebugMessage(const std::string& str)
{
    ...
    if (!charcount)
      return;
    wchar_t* cmdline(new wchar_t[charcount]);
    if (!MultiByteToWideChar(CP_UTF8, 0, str.c_str(), -1, cmdline, charcount)) {
      delete[] cmdline;
      return;
    }
    ...
    if (CreateProcessW(prog_name, cmdline, NULL, NULL, false, 0, NULL, NULL,
...
LogMessage::~LogMessage()
{
    ...
    if (log_filter_prefix && severity_ <= kMaxFilteredLogLevel && str_newline.compare(message_start_, strlen(log_filter_prefix), log_filter_prefix) != 0) {
        return;
    }
    ...
    if (logging_destination == LOG_ONLY_TO_SYSTEM_DEBUG_LOG || logging_destination == LOG_TO_BOTH_FILE_AND_SYSTEM_DEBUG_LOG) {
        ...
        std::cout << str_newline;
    }
    // write to log file
    if (logging_destination != LOG_NONE && logging_destination != LOG_ONLY_TO_SYSTEM_DEBUG_LOG && InitializeLogFileHandle()) {
...
std::ostream& operator<<(std::ostream& out, const wchar_t* wstr)
{
    ...
    int charcount = WideCharToMultiByte(CP_UTF8, 0, wstr, -1, NULL, 0, NULL, NULL);
    if (charcount == 0)
        return out;

    // convert
    char* buf(new char[charcount]);
    WideCharToMultiByte(CP_UTF8, 0, wstr, -1, buf, charcount, NULL, NULL);
    return out << buf;
    delete[] buf;
}

#endif // BPM_DONT_USE_GLOG
