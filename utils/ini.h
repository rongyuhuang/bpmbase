...
///
template <typename T = fini_string_t,
    typename U = fini_string_t,
...
/// Type definition declarations
    ...
    INI(const std::string filename, bool doParse)
        : filename(filename)
    {
    ...
    fini_string_t get(
        const W key,
        const fini_char_t* def = _T("")) // Handle C string default value without casting
    {
                ...
                if (len > 0 && !((len >= 2 && (line[0] == '/' && line[1] == '/')) || (len >= 1 && line[0] == '#'))) // Ignore comment and empty lines
                {
                    ...
                    } else // Key
                    {
                        ...
                        if (skey && svalue) {
                            ...
                            while (isspace(skey[index])) // Leave out any additional new line
                                // characters, not "spaces" as the
                                // name suggests
                                index++;
    ...
    bool saveBinaryExperimental(std::string filename = "")
    {
        ...
        for (typename INI::sectionsit_t i = sections.begin(); i != sections.end();
            ...
            for (typename INI::keysit_t j = i->second->begin(); j != i->second->end();
                ...
                file.write((fini_char_t*)&j->second,
...
template <>
inline fini_string_t Converters::Convert<fini_string_t, fini_string_t>(
    fini_string_t value)
{
    return value;
}
...
template <>
inline fini_string_t Converters::Convert<fini_string_t>(
    const fini_char_t* value)
{
    return value;
}
